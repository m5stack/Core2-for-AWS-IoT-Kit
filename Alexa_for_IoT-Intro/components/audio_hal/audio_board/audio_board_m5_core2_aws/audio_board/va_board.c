    ...
    */
    button_driver_config_t button_driver_config = {
        .button_val = {
            ...
            [BUTTON_EVENT_CUSTOM_1] = ((int64_t)1) << 2,        /* button_right */
...
int va_board_init()
{
    ...
    ESP_LOGI(VA_TAG, "Installing M5Stack Core2 for AWS board i2s driver for speaker");
    ...
    /* The buttons for this board are on the touch display itself. The Core2ForAWS_Init initializer function initializes the screen and virtual
    button drivers if enabled in the configuration. The button actions need to be initialized only after initializing the device. */
