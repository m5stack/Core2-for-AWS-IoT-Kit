...
static bool button_driver_check_value(button_event_t event)
{
    ...
    ESP_LOGI(TAG, "current_button_value: %d, event: %d, button_config.button_val[event]: %lld", current_button_value, event, button_config.button_val[event]);
...
esp_err_t button_driver_init(button_driver_config_t *button_driver_config)
{
	...
	/*M5StackCore2 specific Buttons driver is already initalized in Core2ForAWS_Init()*/
